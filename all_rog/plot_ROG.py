...
with open('DD_distance_traj_2dendrons_ETN25%.pickle','rb') as pickle_file:
    distance=pk.load(pickle_file)


print(distance)

egvl=DiagonalizeTensor()
plt.plot(np.sqrt(egvl[0]))
plt.plot(np.sqrt(egvl[1]))
plt.plot(np.sqrt(egvl[2]))
plt.plot(distance)
plt.show()
